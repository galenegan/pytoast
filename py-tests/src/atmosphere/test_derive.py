...
def test_t_only():
    out = _stub().derive(_burst(keys=("t",)))
...
def test_t_p_rh_full_set():
    out = _stub().derive(_burst(keys=("t", "p", "rh")))
...
def test_sp_forwarded_to_saturation_vapor_pressure():
    ...
    out_no_sp = _stub().derive(burst_no_sp)
    out_sp = _stub().derive(burst_sp)
...
def test_theta_broadcasts_over_heights():
    ...
    out = _stub(n_heights=n_heights).derive(_burst(n_heights=n_heights, n_samples=n_samples, keys=("t",)))
